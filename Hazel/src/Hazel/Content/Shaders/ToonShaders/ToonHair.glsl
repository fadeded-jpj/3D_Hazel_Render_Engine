...
void main()
{
	...
	vec3 color = ambient + CacLighting(baseColor, vec3(0.0), normal, viewDir, v_WorldPos.xyz) + SampleSphereLighting(N);
	...
	vec3 toHead = u_HeadPostion - v_WorldPos.xyz;
	float distance = dot(toHead, toHead);
	float VdotHeadForward = dot(viewDir, u_HeadForward);
	float alphaFactor = smooth(0.5, 1, 1.0 - VdotHeadForward);
	alpha = mix(1.0, 0.5, alphaFactor);
	if (distance > 0.1)
		alpha = 1.0;
