...
// ------------------------------------
#define TOON_SCENE_SHADOW_ONLY
#include "../Common/ToonLighting.glsl"
#include "../Common/ToonCharacterOutput.glsl"



float CalculateEyeVisibility(vec3 fragmentWorldPos)
{
	if (u_HasHeadTransform == 0 ||
		u_ShadowEnabled == 0 ||
		u_ShadowLightIndex < 0 ||
		u_ShadowLightIndex >= u_LightCount)
	{
		return 1.0;
	}

	vec3 headRight = normalize(u_HeadRight);
	vec3 eyeNormal = normalize(-u_HeadForward);

	float side = dot(
		fragmentWorldPos - u_HeadPosition,
		headRight);

	vec3 eyeCenter =
		u_HeadPosition +
		(side >= 0.0 ? 0.1 : -0.1) * headRight;

	RenderLight light = u_Lights[u_ShadowLightIndex];
	...
	{
		lightDir = normalize(lightPosition - eyeCenter);
	}

	return CalculateSceneVisibility(
		eyeCenter,
		eyeNormal,
		lightDir,
		lightType,
...
void main()
{
	...
	// 根据当前fragment 世界坐标， 和 head position， 计算是左眼还是右眼
	float eyeVisibility = CalculateEyeVisibility(v_WorldPos.xyz);

	vec3 color = baseColor * mix(0.6, 0.8, smoothstep(
		0.3, 0.7, eyeVisibility
	));
	//vec3 color = vec3(eyeVisibility);
